...
例えば、サーバーが4つほどあるとして、クライアントは5つあるとロードバランサーが以下のようにマッピングを行う。  
...
コンシステントハッシュ法では`hash(o) mod n`でマッピングする対象を決めている。  
そのため、もしサーバーを後になって増やしたい場合や、一つのサーバーが機能しなくなってしまった場合、**すべてのマッピング結果が変わる可能性がある**。

### Rendezvous hashing

ランデヴーハッシュ法では、ハッシュ化した結果を見て、最も近いもので対象を決めている。
そのため、もしサーバーを後になって増やしたい場合や、一つのサーバーが機能しなくなってしまった場合、**一部のマッピング結果のみが変わる可能性がある**。

## Relational Database

### ACID

ACID特性とは、トランザクション処理求められる4つの特性を示したもの。

- Atomic 原子性
  - トランザクション全体が実行されるか、実行されないかのどちらかであることが保証されること。
- Consistency 一貫性
  - トランザクションの前後でデータの整合性が保たれることで、矛盾がない状態のこと。
- Isolation 独立性
  - トランザクション実行中の内容が他の処理に影響を与えない性質のこと。(排他制御)
- Durability 耐久性
  - トランザクションが完了すると、その結果が失われることがないという性質のこと。

## Specialized Storage Paradigms

### Blob Store

任意の非構造データを保存できる空間のこと。非構造データとは画像や動画などのファイルのこと。  
GCSやS3などがあたる。

### Time Series DB

時系列データベースとは、ログなどの時間を軸とした連続したデータを蓄積、検索することに特化して作られたデータベースのことで、主にIoT分野で発生する大量のセンサーデータや、ログを蓄積する用途において他のデータベースよりも高いパフォーマンスを発揮する。  
InfluxDBやPrometheusなどがあたる。

### Graph DB

グラフデータベースとは、グラフ構造となっているデータベースのことで、リレーショナルデータベースと比べて、データがネットワーク状になっている場合に威力を発揮する。

グラフはノード、エッジ、プロパティの三要素によって表現されている。

- ノード
  - 頂点のこと。ラベル名を保有する。
- エッジ
  - ノード間の関係性のこと。方向と関係性を保有する。
- プロパティ
  - 属性のこと。ノードに付随する情報。key/valueペアのデータ。

### Spatial DB

空間データベースとは、地理的な位置などの空間データを格納するデータベースのこと。  
リレーショナルデータベースではインデックスをして高速な検索を可能にするが、空間クエリには適していない。空間データベースでは空間インデックスというものがあり、これを使用してパフォーマンスを高速化することができる。

## Replication and Sharding

レプリケーションとは、万が一に備えて複製を作ることを意味している。  
レプリケーションを用意することで、メインDBが異常終了したとしても、異常終了前までデータが同期化されたDBがあることで、システムをそのまま稼働させることができる。

シャーディングとは、データベースにパーティションを設けることで、データベース内に複数のデータ領域を用意することで、データベースへのアクセスを分散し、全体のスループットを上げることができる。
シャーディングにおいて保存先の領域はハッシュ化などで決めることができる。

## Leader Election

例えば、複数のサーバーがあり、それらのサーバーのうち1つのサーバーだけがある特定のタスクを担当しなければならないときに、サーバーを決める過程のこと。  
アルゴリズムを一から実装することは少なく、`etcd`などで実装することができる。

## P2P Network

P2Pネットワークとは、複数のコンピューター間で通信を行う際のアーキテクチャ。  
クライアント・サーバー方式では一つのサーバーに対してクライアントが複数ある方式で、クライアントが多くなりすぎるとサーバーに負荷がかかるが、P2Pではクライアント同士で通信を行うことができるため、負荷が分散できる。

## Polling and Streaming

ポーリングとは、クライアントがサーバーに対して毎X秒サーバーに対してリクエストを行うことをいう。

ストリーミングとは、クライアントとサーバー間でコネクションを常時張って、サーバー側からデータをクライアントに送ることをいう。これはSocketを利用することで実現できる。

ポーリングはリアルタイム性が求められていない場合に使うべきで、ストリーミングはチャットアプリケーションなどのリアルタイム性が求められる場合に使える。

## Rate Limiting

レート制限とは、クライアントからサーバーへのリクエストを制限すること。  
例えばクライアントからサーバーへのリクエストを10秒間に2回しか許可しないレート制限をすることで、クライアントが3回目にリクエストを送ったときサーバーからクライアントへ`429 (Too many requests.)`などを送ることで、サーバーが対応できない量のリクエストに対して対策できる。Dos攻撃やWebスクレイピングなどを制限するために使用できる。  
レート制限は様々な方法で実現でき、IPで制限したり、アクセスしているユーザを特定してそのユーザによるアクセスは3個までなど色々な方法で設定できる。
